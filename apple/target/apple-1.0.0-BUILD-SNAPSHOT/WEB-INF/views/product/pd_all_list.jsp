...
<body>
	...
	<div class="prodList_wrap">
		...
		<select class="pd_select">
			<option value="price_row">낮은 가격순으로 보기</option>
			<option value="price_high">높은 가격순으로 보기</option>
			<option value="reg-day">등록일순으로 보기</option>
		</select>
		<div class="pd_box">
			<ul class="prod_wrap">
	...
	<div class="page-btn">
		<c:if test="${pn > 1}">
			<a href="${pageContext.request.contextPath}/product_all_list.ap?pg=${pn-1}" class="page-btn-link"><span class="page-span">이전</span></a>
		</c:if>
		<c:forEach begin="1" end="${maxPg}" step="1" varStatus="vs">
			...
			<c:if test="${vs.current ne pn}">
				<a href="${pageContext.request.contextPath}/product_all_list.ap?pg=${vs.current}" class="page-btn-link"><span class="page-span">${vs.current}</span></a>
			</c:if>
		</c:forEach>
		<c:if test="${pn < maxPg}">
			<a href="${pageContext.request.contextPath}/product_all_list.ap?pg=${pn+1}" class="page-btn-link"><span class="page-span">다음</span></a>
		</c:if>
	</div>
