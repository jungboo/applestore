...
<body>
	...
	<form action="${pageContext.request
		.contextPath}/notice_search.ap" method="post">
	...
	<div class="board_list_wrap">
        ...
        <div id="paginate" class="paging">
			<a class="bt" href="${pageContext.request.contextPath}/notice_list.ap?pg=1">첫 페이지</a>
			<c:if test="${pn > 1}">
				<a class="bt" href="${pageContext.request.contextPath}/notice_list.ap?pg=${pn-1}"
				>이전 prev &lt;&lt;</a>
			</c:if>
				&nbsp;&nbsp;
			<c:forEach begin="1" end="${maxPg}" step="1" 
				...
				<c:if test="${vs.current eq pn}">
					...
					.contextPath}/notice_list.ap?pg=${vs.current}"
					>${vs.current}</a>
					</b> <!-- 현재 페이지 -->
				</c:if>	
			...
			<c:if test="${pn < maxPg}">
				...
				.contextPath}/notice_list.ap?pg=${pn+1}"
				>다음 next &gt;&gt;</a>
			</c:if>	
			<a class="bt" href="${pageContext.request.contextPath}/notice_list.ap?pg=${maxPg}">마지막 페이지</a>
		</div>
    </div>
	</c:if>
